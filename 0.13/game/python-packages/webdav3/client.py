...
# ----------  Ren’Py 精简 lxml 兼容补丁 ----------
import sys, types, xml.etree.ElementTree as _ET

_fake_etree = types.ModuleType('lxml.etree')
_fake_etree.fromstring = _ET.fromstring
_fake_etree.XMLSyntaxError = _ET.ParseError
_fake_etree.Element = _ET.Element
_fake_etree.SubElement = _ET.SubElement
_fake_etree.ElementTree = _ET.ElementTree
...
class Client(object):
    ...
    def download_file(self, remote_path, local_path, progress=None, progress_args=()):
        ...
        with open(local_path, 'wb') as local_file:
            ...
            total = int(response.headers.get('content-length', 0))   # 0 表示未知大小

            current = 0

            if callable(progress) and total > 0:   # 只有知道总大小才回调
                progress(current, total, *progress_args)
